...
OUTPUT_IMG = "/projects/coursework/2026-spring/cs395t-lin/asher/take2/2predmech_2project/useful_pf.png"

# Define the exact file prefixes
PREFIX_BASELINE = "champsim_mockingjay.orig_triage_17_nlpc_llc_filtNone_256_32_prom_lru"
PREFIX_EXP = "champsim_mockingjay.orig_triage_9_lpc_llc_filtAll_256_256_nprom_lru",
...
def get_useful_prefetches(filepath):
    """
    Scans a ChampSim trace output file and extracts the USEFUL prefetch count.
    """
    re_pref = re.compile(r"cpu0->LLC\s+PREFETCH\s+REQUESTED:\s+(\d+)\s+ISSUED:\s+(\d+)\s+USEFUL:\s+(\d+)\s+USELESS:\s+(\d+)")
...
def main():
    ...
    ax.set_ylabel('Percent Increase in Useful Prefetches (%)')
