...
BASELINE_DIR = "/projects/coursework/2026-spring/cs395t-lin/asher/take2/2predmech_2project/new_experiments"
EXP_DIR = "/projects/coursework/2026-spring/cs395t-lin/asher/take2/2predmech_2project/new_experiments"
OUTPUT_IMG = "/projects/coursework/2026-spring/cs395t-lin/asher/take2/2predmech_2project/hit_rate.png"
...
def get_demand_hit_rate(filepath):
    """
    Scans a ChampSim trace output file and extracts LOAD and RFO accesses and hits.
    Returns: (LOAD_HIT + RFO_HIT) / (LOAD_ACCESS + RFO_ACCESS)
    """
    re_load = re.compile(r"cpu0->LLC\s+LOAD\s+ACCESS:\s+(\d+)\s+HIT:\s+(\d+)")
    re_rfo = re.compile(r"cpu0->LLC\s+RFO\s+ACCESS:\s+(\d+)\s+HIT:\s+(\d+)")
    
    stats = {'load_acc': 0.0, 'load_hit': 0.0, 'rfo_acc': 0.0, 'rfo_hit': 0.0}
    ...
    try:
        with open(filepath, 'r') as f:
            for line in f:
                ...
                if m_load:
                    stats['load_acc'] = float(m_load.group(1))
                    stats['load_hit'] = float(m_load.group(2))
                    continue
                
                # 2. Grab RFO Stats
                m_rfo = re_rfo.search(line)
                if m_rfo:
                    stats['rfo_acc'] = float(m_rfo.group(1))
                    stats['rfo_hit'] = float(m_rfo.group(2))
                    continue
    ...
    total_demand_acc = stats['load_acc'] + stats['rfo_acc']
    total_demand_hit = stats['load_hit'] + stats['rfo_hit']
...
def main():
    ...
    # Color the avg bar differently (e.g., purple) to make it stand out, 
    # while keeping the red/blue logic for the normal families
    colors = []
    ...
    ax.set_ylabel('Demand Data Hit Rate Increase (%)')
    ...
    # Give it a little breathing room on top and bottom
