...
# --- PATH CONFIGURATION ---
DIRS_TO_SEARCH = [
    "/projects/coursework/2026-spring/cs395t-lin/asher/take2/2predmech_2project/experiments",
    "/projects/coursework/2026-spring/cs395t-lin/asher/take2/2predmech_2project/new_experiments"
]
OUTPUT_IMG = "/projects/coursework/2026-spring/cs395t-lin/mbd2325/replacement_geomean_tall.png"

# --- EXPERIMENT CONFIGURATION ---
PREFIX_ABS_BASELINE = "champsim_lru_no_17_nlpc_llc_filtAll_256_32_prom_lru"
...
POLICIES = {
    "Mockingjay": {
        "with": "champsim_mockingjay.orig_triage_16_lpc_llc_filtAll_256_32_nprom_lru",
        "without": "champsim_mockingjay.orig_triage_17_nlpc_llc_filtNonLLC_256_32_prom_lru"
    },
    "PACMAN": {
        "with": "champsim_pacman_triage_16_lpc_llc_filtAll_256_32_nprom_lru",
        "without": "champsim_pacman_triage_17_nlpc_llc_filtNonLLC_256_32_prom_lru"
    },
    "SHiP": {
        "with": "champsim_ship_triage_16_lpc_llc_filtAll_256_32_nprom_lru",
        "without": "champsim_ship_triage_17_nlpc_llc_filtNonLLC_256_32_prom_lru"
    },
    "LRU": {
        "with": "champsim_lru_triage_16_lpc_llc_filtAll_256_32_nprom_lru",
        "without": "champsim_lru_triage_17_nlpc_llc_filtNonLLC_256_32_prom_lru"
    }
}
...
def main():
    ...
    for d in DIRS_TO_SEARCH:
        for filepath in glob.glob(os.path.join(d, f"{PREFIX_ABS_BASELINE}*")):
            ...
            if ipc: baseline_ipcs[trace] = ipc
        for name, configs in POLICIES.items():
            for cfg_type, prefix in configs.items():
                for filepath in glob.glob(os.path.join(d, f"{prefix}*")):
                    ...
                    ipc = get_ipc(filepath)
                    if ipc: policy_results[name][cfg_type][trace] = ipc

    # 2. Intersection logic & Diagnostics
    all_known_traces = set(baseline_ipcs.keys())
    for p in POLICIES:
        for cfg in ["with", "without"]:
            all_known_traces.update(policy_results[p][cfg].keys())
    ...
    # --- DIAGNOSTIC OUTPUT ---
    dropped = all_known_traces - common_traces
    if dropped:
        print(f"\n[!] WARNING: Dropped {len(dropped)} trace(s) due to missing data. Identifying culprits...")
        for trace in sorted(list(dropped)):
            print(f"\nTrace: {trace}")
            if trace not in baseline_ipcs:
                print(f"  ❌ Missing in Baseline ({PREFIX_ABS_BASELINE})")
            for p in POLICIES:
                for cfg in ["with", "without"]:
                    if trace not in policy_results[p][cfg]:
                        prefix = POLICIES[p][cfg]
                        print(f"  ❌ Missing in {p} [{cfg} LPC] ({prefix})")
        print("-" * 50)
    else:
        print("\n[+] Perfect match: No traces dropped!")
